                ...
                if command_word in word:
                    ...
                    if (wager_value < minimum_bet):
                        ...
                        from_comment.blackjack_result = format_all(
                            player_hand, dealer_hand, rest_of_deck, status, wager, currency_prop, 0)


def player_hit(from_comment, did_double_down=False):
    player_hand, dealer_hand, deck, status, wager, kind, is_insured = from_comment.blackjack_result.split(
        "_")
    ...
    from_comment.blackjack_result = format_all(
        player_hand, dealer_hand, deck, status, wager, kind, int(is_insured))
...
def player_doubled_down(from_comment):
    ...
    deck = deck.split("/")
    from_comment.blackjack_result = format_all(
        player_hand, dealer_hand, deck, status, str(wager_value), kind, int(is_insured))
